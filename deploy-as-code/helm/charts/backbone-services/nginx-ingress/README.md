...
[nginx-ingress](https://github.com/kubernetes/nginx-ingress) Ingress controller for Kubernetes using NGINX as a reverse proxy and load balancer
...
```console
helm repo add nginx-ingress https://kubernetes.github.io/nginx-ingress
helm repo update
```
...
```console
helm install [RELEASE_NAME] nginx-ingress/nginx-ingress
```
...
By default the nginx-ingress controller has service interruptions whenever it's pods are restarted or redeployed. In order to fix that, see the excellent blog post by Lindsay Landry from Codecademy: [Kubernetes: Nginx and Zero Downtime in Production](https://medium.com/codecademy-engineering/kubernetes-nginx-and-zero-downtime-in-production-2c910c6a5ed8).

### Migrating from stable/nginx-ingress

There are two main ways to migrate a release from `stable/nginx-ingress` to `nginx-ingress/nginx-ingress` chart:
...
Note that there are some different and upgraded configurations between the two charts, described by Rimas Mocevicius from JFrog in the "Upgrading to nginx-ingress Helm chart" section of [Migrating from Helm chart nginx-ingress to nginx-ingress](https://rimusz.net/migrating-to-nginx-ingress). As the `nginx-ingress/nginx-ingress` chart continues to update, you will want to check current differences by running [helm configuration](#configuration) commands on both charts.
...
- In [0.16.1](https://github.com/kubernetes/nginx-ingress/blob/main/Changelog.md#0161), the vts (virtual host traffic status) dashboard was removed
- In [0.23.0](https://github.com/kubernetes/nginx-ingress/blob/main/Changelog.md#0230), the status page at port 18080 is now a unix socket webserver only available at localhost.
  You can use `curl --unix-socket /tmp/nginx-status-server.sock http://localhost/nginx_status` inside the controller container to access it locally, or use the snippet from [nginx-ingress changelog](https://github.com/kubernetes/nginx-ingress/blob/main/Changelog.md#0230) to re-enable the http server
...
Annotate the controller as shown in the [nginx-ingress l7 patch](https://github.com/kubernetes/nginx-ingress/blob/ab3a789caae65eec4ad6e3b46b19750b481b6bce/deploy/aws/l7/service-l7.yaml):
...
With nginx-ingress-controller in 0.25.* work only with kubernetes 1.14+, 0.26 fix [this issue](https://github.com/kubernetes/nginx-ingress/pull/4521)
...
|-----|------|---------|-------------|
| commonLabels | object | `{}` |  |
| controller.addHeaders | object | `{}` | Will add custom headers before sending response traffic to the client according to: https://kubernetes.github.io/nginx-ingress/user-guide/nginx-configuration/configmap/#add-headers |
...
| controller.admissionWebhooks.patch.image.image | string | `"nginx-ingress/kube-webhook-certgen"` |  |
...
| controller.config | object | `{}` | Will add custom configuration options to Nginx https://kubernetes.github.io/nginx-ingress/user-guide/nginx-configuration/configmap/ |
...
| controller.image.image | string | `"nginx-ingress/controller"` |  |
...
| controller.ingressClassResource.controllerValue | string | `"k8s.io/nginx-ingress"` | Controller-value of the controller that is processing this ingressClass |
...
| controller.opentelemetry.image | string | `"registry.k8s.io/nginx-ingress/opentelemetry:v20230107-helm-chart-4.4.2-2-g96b3d2165@sha256:331b9bebd6acfcd2d3048abbdd86555f5be76b7e3d0b5af4300b04235c6056c9"` |  |
| controller.podAnnotations | object | `{}` | Annotations to be added to controller pods # |
| controller.podLabels | object | `{}` | Labels to add to the pod container metadata |
| controller.podSecurityContext | object | `{}` | Security Context policies for controller pods |
| controller.priorityClassName | string | `""` |  |
| controller.proxySetHeaders | object | `{}` | Will add custom headers before sending traffic to backends according to https://github.com/kubernetes/nginx-ingress/tree/main/docs/examples/customization/custom-headers |
...
| dhParam | string | `nil` | A base64-encoded Diffie-Hellman parameter. This can be generated with: `openssl dhparam 4096 2> /dev/null | base64` # Ref: https://github.com/kubernetes/nginx-ingress/tree/main/docs/examples/customization/ssl-dh-param |
...
| tcp | object | `{}` | TCP service key-value pairs # Ref: https://github.com/kubernetes/nginx-ingress/blob/main/docs/user-guide/exposing-tcp-udp-services.md # |
| udp | object | `{}` | UDP service key-value pairs # Ref: https://github.com/kubernetes/nginx-ingress/blob/main/docs/user-guide/exposing-tcp-udp-services.md # |
